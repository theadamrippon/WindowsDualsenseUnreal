...
bool DualSenseHIDManager::FindDevices(TArray<FHIDDeviceContext>& Devices)
{
    ...
    for (DWORD DeviceIndex = 0; SetupDiEnumDeviceInterfaces(DeviceInfoSet, nullptr, &HidGuid, DeviceIndex, &DeviceInterfaceData); DeviceIndex++)
    {
        ...
        if (SetupDiGetDeviceInterfaceDetail(DeviceInfoSet, &DeviceInterfaceData, DetailDataBuffer, RequiredSize, nullptr, nullptr))
        {
            UE_LOG(LogTemp, Log, TEXT("HIDManager: Detalhes do dispositivo encontrados: %s"), DetailDataBuffer->DevicePath);
            ...
            if (TempDeviceHandle != INVALID_HANDLE_VALUE)
            {
                ...
                if (HidD_GetAttributes(TempDeviceHandle, &Attributes))
                {
                    ...
                    {
                        if (!HidD_GetProductString(TempDeviceHandle, Context.Internal.DevicePath, sizeof(Context.Internal.DevicePath)))
                        {
...
bool DualSenseHIDManager::GetDeviceInputState(FHIDDeviceContext* DeviceContext, unsigned char* InputState)
{
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("GetDeviceInputState: Dispositivo não está conectado ou o contexto é inválido. %d"), DeviceContext->Internal.Connected);
...
bool DualSenseHIDManager::ReconnectDevice(FHIDDeviceContext* DeviceContext)
{
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("Falha ao reconectar ao dispositivo DualSense. Código de erro: %lu"), GetLastError());
        return false;
    }
...
void DualSenseHIDManager::OutputBuffering(FHIDDeviceContext* Context, const FOutputBuffer& HidOut)
{
    ...
    {
        TriggerL[0x1] = HidOut.LeftTrigger.StartPosition;
        TriggerL[0x2] = HidOut.RightTrigger.KeepEffect;
        TriggerL[0x4] = HidOut.LeftTrigger.Strengths.Start;
    ...
    if (HidOut.LeftTrigger.Mode == 0x22) // Bow
    {
        ...
        TriggerL[0x2] = HidOut.LeftTrigger.EndPosition;
        TriggerL[0x3] = HidOut.LeftTrigger.Strengths.Start;
        TriggerL[0x4] = HidOut.LeftTrigger.Strengths.End;
    }

    if (HidOut.LeftTrigger.Mode == 0x23) // TODO Galope Effect
    {
        TriggerL[0x1] = HidOut.LeftTrigger.StartPosition;
        TriggerL[0x2] = HidOut.LeftTrigger.EndPosition;
        TriggerL[0x3] = HidOut.LeftTrigger.TimeRatio;
    ...
    if (HidOut.RightTrigger.Mode == 0x22) // Bow
    {
        ...
        TriggerR[0x2] = HidOut.RightTrigger.EndPosition;
        TriggerR[0x3] = HidOut.RightTrigger.Strengths.Start;
        TriggerR[0x4] = HidOut.RightTrigger.Strengths.End;
    }
    ...
    if (HidOut.LeftTrigger.Mode == 0x23) // TODO Galope Effect
    {
        TriggerR[0x1] = HidOut.RightTrigger.StartPosition;
        TriggerR[0x2] = HidOut.RightTrigger.EndPosition;
        TriggerR[0x3] = HidOut.RightTrigger.TimeRatio;
    ...
    {
        TriggerR[0x1] = HidOut.RightTrigger.StartPosition;
        TriggerR[0x2] = HidOut.RightTrigger.KeepEffect;
        TriggerR[0x4] = HidOut.RightTrigger.Strengths.Start;
    ...
    if (!WriteFile(Context->Internal.DeviceHandle, Context->Internal.Buffer, sizeof(Context->Internal.Buffer), &BytesWritten, nullptr))
    {
